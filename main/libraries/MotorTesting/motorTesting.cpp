...
void Motors::slowDownLeft() {
    /* Set speed on left to half of full speed */
    TC0->TC_CHANNEL[0].TC_RA = TC_DUTY_MOVING_SLOW;
}

void Motors::slowDownRight() {
    /* Set speed on right to half of full speed */
    PWM->PWM_CH_NUM[0].PWM_CDTY = PWM_DUTY_MOVING_SLOW;
}

void Motors::slowDown() {
    /* Set speed to half of full speed */
    slowDownLeft();
    slowDownRight();
}

void Motors::turn45(bool left) {
    /* Turn left or right 45 degrees using encoders and turn time */
    if (left) {
        moveForwardRight();
        moveReverseLeft();
    } else {
    ...
    turnEncoderTicks = TURN_ENCODER_TICKS_45;
    state = TURNING;
    encoders.ResetTotalCounts();
    encoders.Enable();
    Timer4.attachInterrupt(TurnISR).start(DEFAULT_TURN_45_TIME_MS*1000);
}


void Motors::turn90(bool left) {
    /* Turn left or right 90 degrees using encoders and turn time */
    if (left) {
        moveForwardRight();
        moveReverseLeft();
    } else {
    ...
    turnEncoderTicks = TURN_ENCODER_TICKS_90;
    state = TURNING;
    encoders.ResetTotalCounts();
    encoders.Enable();
    Timer4.attachInterrupt(TurnISR).start(DEFAULT_TURN_90_TIME_MS*1000);
}


void Motors::turn180(bool left) {
    /* Turn left or right 180 degrees using encoders and turn time */
    state = TURNING;
    if (left) {
        moveForwardRight();
        moveReverseLeft();
    ...
    turnEncoderTicks = TURN_ENCODER_TICKS_180;
    state = TURNING;
    encoders.ResetTotalCounts();
    encoders.Enable();
    Timer4.attachInterrupt(TurnISR).start(DEFAULT_TURN_180_TIME_MS*1000); 
}

void Motors::CancelTurn() {
    /* Stop the turn and encoders */
    stop();
    Timer4.stop();
    encoders.Disable();
}
...
void TurnISR() {
    /* Check if vehicle is done its turn. If not increase check frequency */
    if (encoders.GetTotalCount() >= Vehicle.turnEncoderTicks) {
        Vehicle.CancelTurn(); 
    } else {
        Timer4.stop();
        Timer4.start(TURN_CHECK_TIME_MS*1000);
    }
}